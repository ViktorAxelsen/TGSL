...
class AttnModel(torch.nn.Module):
    ...
    def forward(self, src, src_t, seq, seq_t, seq_e, mask, weight=None):
        ...
        if weight is not None:
            weight = weight.unsqueeze(2).expand_as(v)
            v = v * weight
...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat, attn_mode='prod', use_time='time', agg_method='attn', num_layers=3,
                 n_head=4, null_idx=0, drop_out=0.1, seq_len=None):
        ...
        if use_time == 'time':
            ...
            self.time_encoder = GraphMixerTE(time_dim=self.n_feat_th.shape[1])
        elif use_time == 'pos':
    ...
    def contrast(self, src_idx_l, target_idx_l, background_idx_l, cut_time_l, num_neighbors=20, full_aug_edge_weight=None):
        src_embed = self.tem_conv(src_idx_l, cut_time_l, self.num_layers, num_neighbors, full_aug_edge_weight=full_aug_edge_weight)
        target_embed = self.tem_conv(target_idx_l, cut_time_l, self.num_layers, num_neighbors, full_aug_edge_weight=full_aug_edge_weight)
        background_embed = self.tem_conv(background_idx_l, cut_time_l, self.num_layers, num_neighbors, full_aug_edge_weight=full_aug_edge_weight)
    ...
    def tem_conv(self, src_idx_l, cut_time_l, curr_layers, num_neighbors=20, full_aug_edge_weight=None):
        ...
        if curr_layers == 0:
            ...
            if full_aug_edge_weight is not None:
                src_ngh_eidx_batch = src_ngh_eidx_batch.reshape(-1) - 1
                aug_edge_weight = full_aug_edge_weight[src_ngh_eidx_batch]
                aug_edge_weight = aug_edge_weight.reshape(-1, num_neighbors)

            # print(src_ngh_eidx_batch.shape)
            # print(src_ngn_edge_feat.shape)
            ...
            local, weight = attn_m(src_node_conv_feat, 
                                   ...
                                   src_ngn_edge_feat, 
                                   mask,
                                   weight=aug_edge_weight if full_aug_edge_weight is not None else None)
            return local