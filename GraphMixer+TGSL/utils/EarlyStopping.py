...
class EarlyStopping(object):
    ...
    def __init__(self, patience: int, save_model_folder: str, save_model_name: str, logger: logging.Logger, model_name: str = None, prefix=None, tolerance=1e-3):
        """
        Early stop strategy.
        ...
        self.logger = logger
        self.save_model_path = os.path.join(save_model_folder, f"{save_model_name}-{prefix}.pkl")
    ...
    def step(self, metrics: list, model: nn.Module):
        ...
        for metric_tuple in metrics:
            ...
            if higher_better:
                if self.best_metrics.get(metric_name) is None or (metric_value - self.best_metrics.get(metric_name)) / np.abs(self.best_metrics.get(metric_name)) >= self.tolerance:
