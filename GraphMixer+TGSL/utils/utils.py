...
class NeighborSampler:
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        nodes_neighbor_ids = np.zeros((len(node_ids), num_neighbors)).astype(int)
        ...
        # ndarray, shape (batch_size, num_neighbors)
        nodes_edge_ids = np.zeros((len(node_ids), num_neighbors)).astype(int)
...
def get_neighbor_sampler(data: pd.DataFrame, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    ...
    return NeighborSampler(adj_list=adj_list, sample_neighbor_strategy=sample_neighbor_strategy, time_scaling_factor=time_scaling_factor, seed=seed), adj_list
...
class NegativeEdgeSampler(object):
    ...
    def historical_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        return negative_src_node_ids.astype(int), negative_dst_node_ids.astype(int)
