...
class MTL(nn.Module):
    ...
    def forward(self, src_l_cut, dst_l_cut, src_l_fake, dst_l_fake, ts_l_cut, eid_l_cut, NUM_NEIGHBORS, time_gap, g, adj_list, adj_list_pickle):
        batch_src_node_embeddings, batch_dst_node_embeddings = self.encoder[0].compute_src_dst_node_temporal_embeddings(src_node_ids=src_l_cut, dst_node_ids=dst_l_cut, node_interact_times=ts_l_cut, num_neighbors=NUM_NEIGHBORS, time_gap=time_gap)
        batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = self.encoder[0].compute_src_dst_node_temporal_embeddings(src_node_ids=src_l_fake, dst_node_ids=dst_l_fake, node_interact_times=ts_l_cut, num_neighbors=NUM_NEIGHBORS, time_gap=time_gap)
        pos_score = self.encoder[1](input_1=batch_src_node_embeddings, input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
        neg_score = self.encoder[1](input_1=batch_neg_src_node_embeddings, input_2=batch_neg_dst_node_embeddings).squeeze(dim=-1).sigmoid()
        ...
        neighbor_node_idx, neighbor_edge_idx, neighbor_ts = neighbor_finder.get_historical_neighbors(train_node_cut, max_ts, num_neighbors=self.rnn_nn)
        ...
        if self.can_type == '1st':
            candidate_node_idx, candidate_edge_idx, candidate_ts = neighbor_finder.get_historical_neighbors(train_node_cut, max_ts, num_neighbors=self.can_nn)
            ...
            candidate_node_idx, candidate_edge_idx, candidate_ts = neighbor_finder.get_multi_hop_neighbors(3, train_node_cut, max_ts, num_neighbors=self.can_nn)
            ...
            candidate_node_idx_1st, candidate_edge_idx_1st, candidate_ts_1st = neighbor_finder.get_historical_neighbors(train_node_cut, max_ts, num_neighbors=self.can_nn)
            candidate_node_idx_3rd, candidate_edge_idx_3rd, candidate_ts_3rd = neighbor_finder.get_multi_hop_neighbors(3, train_node_cut, max_ts, num_neighbors=self.can_nn)
        ...
        train_ngh_finder_aug = NeighborSampler(adj_list_aug, sample_neighbor_strategy=self.encoder[0].neighbor_sampler.sample_neighbor_strategy, seed=self.encoder[0].neighbor_sampler.seed)
        ...
        edge_raw_embed_aug = torch.from_numpy(e_feat_aug.astype(np.float32)).to(self.device)

        ngh_finder_ori = self.encoder[0].neighbor_sampler
        self.encoder[0].set_neighbor_sampler(train_ngh_finder_aug)
        edge_raw_embed_ori = self.encoder[0].edge_raw_features
        self.encoder[0].edge_raw_features = edge_raw_embed_aug

        max_ts = np.array([self.max_train_ts_l + 1] * len(train_node_cut))

        batch_src_node_embeddings, batch_dst_node_embeddings = self.encoder[0].compute_src_dst_node_temporal_embeddings(src_node_ids=src_l_cut, dst_node_ids=dst_l_cut, node_interact_times=ts_l_cut, num_neighbors=NUM_NEIGHBORS, time_gap=time_gap, full_aug_edge_weight=full_aug_edge_weight)
        batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = self.encoder[0].compute_src_dst_node_temporal_embeddings(src_node_ids=src_l_fake, dst_node_ids=dst_l_fake, node_interact_times=ts_l_cut, num_neighbors=NUM_NEIGHBORS, time_gap=time_gap, full_aug_edge_weight=full_aug_edge_weight)
        pos_score_ed = self.encoder[1](input_1=batch_src_node_embeddings, input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
        neg_score_ed = self.encoder[1](input_1=batch_neg_src_node_embeddings, input_2=batch_neg_dst_node_embeddings).squeeze(dim=-1).sigmoid()

        q = self.encoder[0].compute_node_temporal_embeddings(train_node_cut, max_ts, NUM_NEIGHBORS, time_gap, full_aug_edge_weight=full_aug_edge_weight)
        q = nn.functional.normalize(q, dim=1)

        # Recover
        self.encoder[0].set_neighbor_sampler(ngh_finder_ori)
        self.encoder[0].edge_raw_features = edge_raw_embed_ori

        with torch.no_grad():  # no gradient to keys
            self._momentum_update_key_encoder()  # update the key encoder
            k = self.encoder_k[0].compute_node_temporal_embeddings(train_node_cut, max_ts, NUM_NEIGHBORS, time_gap)
        ...
        return pos_score, neg_score, pos_score_ed, neg_score_ed, logits, labels
